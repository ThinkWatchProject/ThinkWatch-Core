//! 每家上游的典型首字节时间（DESIGN.md §4.6、§3.5 的 `url-test`）。
//!
//! # 为什么是 TTFB 而不是总时长
//!
//! 总时长里最大的一项是「模型说了多少字」，那和上游快不快无关 ——
//! 一个回答长的请求会让最快的上游看起来最慢。**TTFB 才是这一层能控制、
//! 用户也真的在等的那段**（§4.6 的 L3 也是这个判据）。
//!
//! # 为什么是中位数而不是 EWMA
//!
//! EWMA 的问题是**说不清**：用户问「为什么切到乙了」，我们只能给一个
//! 被历史加权过的数，而它对应不到任何一次真实的请求。中位数返回的
//! 永远是一个**真实发生过的值**，用户能在请求列表里找到它 —— 和 §4.3
//! 的分位数用最近秩法是同一条理由。
//!
//! 这也是为什么 §4.3 那轮说「EWMA 有意不做」：那时它没有消费者。
...
impl Latency {
    ...
    /// 启动时用 L1 握手的结果垫一个底（§3.5：样本不够时用零成本的 L1 补）。
    ///
