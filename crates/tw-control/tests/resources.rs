...
fn relay(name: &str) -> serde_json::Value {
    serde_json::json!({
        ...
        "key": { "kind": "key", "value": "sk-relay#1: x" },
        "protocol": "anthropic",
    })
}
...
async fn a_new_upstream_lands_with_its_key_intact_and_every_comment_kept() {
    ...
    assert!(matches!(&p.key, tw_config::Secret::Literal(k) if k == "sk-relay#1: x"));
...
#[tokio::test]
async fn the_overview_says_what_kind_of_credential_it_is_and_never_the_value() {
    let b = bed(BASE);
    call(
        &b.app,
        "POST",
    ...
    assert_eq!(official["key_kind"], "key");
    ...
    assert_eq!(env["key_kind"], "env");
    assert_eq!(env["key_env"], "RELAY_KEY");
}
...
async fn a_proxy_password_is_written_intact_kept_on_edit_and_never_shown() {
    ...
    assert!(matches!(&auth.pass, tw_config::Secret::Literal(p) if p == "p@ss #1: x"));
...
async fn an_unsaved_oauth_credential_is_not_refreshed_just_to_test_it() {
    ...
    let (_, body) = call(
        ...
        serde_json::json!({ "provider": {
            ...
            "key": { "kind": "oauth", "refresh": "rt", "endpoint": "https://auth.example/token" },
        }}),
    )
    .await;
    let v = json(&body);
