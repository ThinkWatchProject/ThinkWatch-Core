...
pub fn spawn_watcher(
    ...
    tokio::spawn(async move {
        ...
        let scan_now = |dir: &std::path::Path, home: &std::path::Path| {
            let (rules, _) = tw_scan::rules::load(dir);
            tw_scan::report::scan(&tw_scan::sources::user_level(home), &rules)
        };
        seen.diff(&scan_now(&dir, &home).findings);

        while rx.recv().await.is_some() {
            // **每次重新枚举来源**：用户可能刚加了一个 skill，
            // 而那个文件在启动时还不存在
            let fresh = seen.diff(&scan_now(&dir, &home).findings);
            if fresh.is_empty() {
                continue;
            }
