...
impl Kind {
    ...
    pub fn why(&self) -> &'static str {
        match self {
            Kind::ZeroWidth => "零宽字符：你看不见它，模型看得见。",
            Kind::Tag => {
                "Unicode 标签字符：在任何编辑器里都完全不可见，但会原样进入模型的上下文。一整段指令可以藏在这里。"
            }
            Kind::Bidi => "双向控制符：能让屏幕上的显示顺序和实际字节顺序不一致。",
            Kind::Homoglyph => "同形异义字：长得像拉丁字母，其实不是。常用来伪装命令和域名。",
            Kind::PrivateUse => "私用区码位：没有标准含义，出现在指令文件里本身就可疑。",
        }
    }
}
